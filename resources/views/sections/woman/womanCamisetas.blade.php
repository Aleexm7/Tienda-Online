...
@section('womanCamisetas')

<div class="container">
    <div class="d-flex flex-row mt-4">
        <span class="p-2 ms-2"><a href="{{ route('womanSection')}}" class="nav-link-list">Volver</a></span>
        <span class="p-2 ms-2"><a href="{{ route('womanCamisetas') }}" class="nav-link-list active">Básicas</a></span>
    </div>
    <div class="row mt-4">
        @include('alerts.msg')
        ...
        <div class="col-md-4 mb-4">
            <form action="{{ route('add') }}" method="post">
                ...
                @php
                            ...
                            $imagePath = '/assets/img/products/woman/camisetas/camiseta' . ($loop->index + 1) . '.webp';
                            // Ruta alternativa de la imagen
                            $newImagePath = '/assets/img/products/woman/camisetas/productosNuevos/productoNuevo' . ($loop->index + 1) . '.jpg';
                        @endphp
                        
                        @if (file_exists(public_path($imagePath)))
                            <img src="{{ asset($imagePath) }}" alt="camiseta">
                        @else (file_exists(public_path($newImagePath)))
                            <img src="{{ asset($newImagePath) }}" alt="camiseta nuevo">
                        @endif
                    <div class="card-body">
